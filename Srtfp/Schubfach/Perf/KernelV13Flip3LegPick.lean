/- flip3 spec-proof, pickNearer fallback leg. Split per-module for bounded peak RAM. -/
...
/-- `flip3` correctness, fallback leg: neither high candidate is in
the interval; `pickNearer` decides at the low scale. -/
theorem flip3_some_eq_packed_pick
    (m : Nat) (q : Int) (sUo : UInt64) (ko : Int)
    (hopt : shortestUnsigned_u64_opt_flip3 m q = some (sUo, ko))
    (hs10 : shiftedSig m q (kOfMQ m q) ≥ 10)
    (hIn_f : inRoundingInterval (shiftedSig m q (kOfMQ m q) / 10)
      (kOfMQ m q + 1) m q (isIrregular m q) = false)
    (hIn1_f : inRoundingInterval (shiftedSig m q (kOfMQ m q) / 10 + 1)
      (kOfMQ m q + 1) m q (isIrregular m q) = false) :
  ...
  have hT := of_decide_eq_false (huV ▸ hIn_f)
  ...
  have hT2 := of_decide_eq_false (hwV ▸ hIn1_f)
