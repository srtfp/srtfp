...
/-- `flip3` correctness, `wV`-accept leg: `sHigh` misses but
`sHigh + 1` is in the rounding interval. -/
theorem flip3_some_eq_packed_wV
    ...
    (hIn1 : inRoundingInterval (shiftedSig m q (kOfMQ m q) / 10 + 1)
  ...
  have hT2 := of_decide_eq_true (hwV ▸ hIn1)
