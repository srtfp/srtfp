...
/-- `flip3` correctness, `uV`-accept leg: `sHigh` itself is in the
rounding interval. -/
theorem flip3_some_eq_packed_uV
    (m : Nat) (q : Int) (sUo : UInt64) (ko : Int)
    (hopt : shortestUnsigned_u64_opt_flip3 m q = some (sUo, ko))
    (hs10 : shiftedSig m q (kOfMQ m q) ≥ 10)
    (hIn : inRoundingInterval (shiftedSig m q (kOfMQ m q) / 10)
  ...
  have hT := of_decide_eq_true (huV ▸ hIn)
