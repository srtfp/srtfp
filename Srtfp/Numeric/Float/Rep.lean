/- FloatRep — rational-valued model of IEEE-754 binary64.
   ...
   The Schubfach printer (`PP.Numeric.Schubfach`) and Clinger reader
   (`PP.Numeric.Clinger`) operate on raw `Float`s, which Lean exposes only
...
NaN / Infinity. The qualified name is `PP.Numeric.Float.Float.toFloatRep`
(so it can be invoked as `Float.toFloatRep` from within the
`PP.Numeric.Float` namespace). -/
def _root_.PP.Numeric.Float.Float.toFloatRep
...
theorem toFloatRep_eq_decode
    ...
    (h : _root_.PP.Numeric.Float.Float.toFloatRep f = some r) :
    r.sign = (decode f).sign ∧ r.m = (decode f).m ∧ r.q = (decode f).q := by
  unfold _root_.PP.Numeric.Float.Float.toFloatRep at h
  split at h
  · simp at h
...
theorem toFloatRep_isSome_iff (f : _root_.Float) :
    (_root_.PP.Numeric.Float.Float.toFloatRep f).isSome ↔ biasedExpBits f < 2047 := by
  unfold _root_.PP.Numeric.Float.Float.toFloatRep
...
theorem toFloatRep_isSome_iff_finite (f : _root_.Float) :
    (_root_.PP.Numeric.Float.Float.toFloatRep f).isSome ↔ isFiniteBits f = true := by
