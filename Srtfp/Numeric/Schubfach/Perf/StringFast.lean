...
open PP.Numeric.Decimal (canonicaliseAux)
...
def decimalToStrRef (d : _root_.PP.Numeric.Decimal) : String :=
...
private theorem decimalToStrRef_mk' (sign : Bool) (sig : Nat) (exp : Int) :
    decimalToStrRef (_root_.PP.Numeric.Decimal.mk' sign sig exp)
  ...
  unfold decimalToStrRef _root_.PP.Numeric.Decimal.mk' _root_.PP.Numeric.Decimal.canonical
  by_cases hs0 : sig = 0
  · simp [hs0]
...
theorem toStringFast_eq_ref (f : _root_.Float) : toStringFast f = floatToStrRef f := by
  ...
  show (if sig = 0 then (if (decode f).sign then "-0" else "0")
          ...
          if sig' = 0 then (if (decode f).sign then "-0" else "0")
            ...
            let signStr := if (decode f).sign then "-" else ""
            signStr ++ toString sig' ++ "e" ++ intToStrRef exp')
      = decimalToStrRef (_root_.PP.Numeric.Decimal.mk' (decode f).sign sig exp)
